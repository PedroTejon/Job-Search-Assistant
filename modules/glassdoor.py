...
cookies_json = load(open('data/cookies.json', 'r',
                    encoding='utf-8'))['glassdoor']
COOKIES = ';'.join(
    [f"{cookie['name']}={cookie['value']}" for cookie in cookies_json])
token = load(open('data/local_storage.json', 'r',
             encoding='utf-8'))['glassdoor_csrf']
...
def get_companies_listings():
    for company in filter(lambda x: x.platforms['glassdoor']['name'] not in [None, 'not_found']
        ...
        company.platforms['glassdoor']['last_check'] = now().strftime(
            "%Y-%m-%dT%H:%M:%S")
        company.save()
...
def get_listing_details(listing):
    ...
    listing.application_url = 'https://www.glassdoor.com.br/' + \
        content['header']['jobLink']
