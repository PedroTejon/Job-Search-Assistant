...
with open('data/cookies.json', 'rb') as cookies_f:
    cookies_json = load(cookies_f)['glassdoor']
with open('data/local_storage.json', 'rb') as token_f:
    token = load(token_f)['glassdoor_csrf']
...
def extract_job_listings(job_listings: list) -> None:
    for listing in job_listings:
        listing_id: str = listing['jobview']['job']['listingId']
        if listing_exists(listing_id):
            continue
        listing_header: dict = listing['jobview']['header']
        listing_title: str = listing_header['jobTitleText']
        listing_location: str = listing_header['locationName']
        listing_worktype: Literal['Remoto', 'Presencial/Hibrido'] = (
            'Remoto' if 'remoto' in listing_location else 'Presencial/Hibrido'
        )
