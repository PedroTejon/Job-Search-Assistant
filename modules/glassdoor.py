...
def get_csrf_token() -> str:
    local_storage = load(open('data/local_storage.json', 'r', encoding='utf-8'))
    return local_storage['glassdoor_csrf']
...
def extract_job_listings(job_listings: list):
    for listing in job_listings:
        ...
        if filter_listing(sub(r'[\[\]\(\),./\\| ]+', ' ', unidecode(listing_title).lower()), listing_location, listing_worktype):
            if (company := get_company_by_name(company_name)).platforms['glassdoor']['name'] == None:
...
def scrape_recommended_listings():
    ...
    for page in range(20):
        ...
        cursor = list(filter(lambda x: x['pageNumber'] > page + 1, content['jobListings']['paginationCursors']))
        if not cursor:
            break
        else:
            cursor = cursor[0]
...
def get_followed_companies():
    ...
    for company in filter(lambda x: x.platforms['glassdoor']['id'] == None and x.followed, companies):
        session = create_scraper(browser={
            'browser': 'firefox',
            'platform': 'windows',
            'mobile': False
        ...
        request = session.get(f'http://www.glassdoor.com.br/api-web/employer/find.htm?autocomplete=true&term={company.platforms["linkedin"]["name"]}', 
            headers={
                'cookie': cookies
            })
        ...
        if len(response):
            company.platforms['glassdoor']['id'] = str(response[0]['id'])
            company.platforms['glassdoor']['name'] = response[0]['label']
        else:
